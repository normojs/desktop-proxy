...
| `config set <key> <value>` | 设置配置键（`logLevel`/`stealth`/`safeMode`/`autoUpdate`/`enforcePermissions`/`maxResponseBodyBytes`/`cdpNetwork`）。 |
...
| `api.network` | `onRequest` / `onResponse` 拦截钩子。响应体非阻塞读取（流式安全）、按 `maxResponseBodyBytes`（默认 1 MiB）截断、二进制类型跳过。事件带 `source` 标签。主作用域插件还能观察到 `webRequest` 看不到的 Node `http`/`https` 流量；开启 `cdpNetwork` 后还能经 CDP 观察到**所有渲染层请求**（`contextIsolation` 下也有效）。 |
