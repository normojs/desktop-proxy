...
name `desktop-proxy` and the short alias **`dprox`** (e.g. `dprox install`, `dprox pair`).
...
| `api.events` | Pub/sub bus (`on(topic)` / `emit(topic, data)`) routed through main, spanning main ↔ all renderers ↔ plugins. The framework publishes `config:changed` / `plugins:changed`. Runs over a unified message router (one protocol for events **and** RPC, carried over Electron IPC in-app and optionally NATS for a paired phone/CLI). The built-in pages and `api.fs` use the same router; a main-side allowlist (`config`/`plugin`/`traffic` methods) gates what a remote peer may call, and denied remote RPCs fail fast with `forbidden`. |
...
| `api.fs` | Sandboxed file I/O confined to the plugin's data dir: `read` / `write` / `exists` / `list` / `delete` / `mkdir` / `stat` (utf8 or base64). Routed over the unified bus but **in-app only** — never in the remote allowlist, so a paired phone/CLI cannot reach the desktop filesystem. |
