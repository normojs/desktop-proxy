 ...
 * Vendored from @desktop-proxy/remote-core so the daemon can serve chat.* RPCs
 * (the heavy parsing stays in Node; the phone just renders the turns).
 *
 ...
 * stable key (system + first user text), then expose turns plus aggregate cost.
 */
