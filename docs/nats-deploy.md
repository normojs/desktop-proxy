...
`scripts/nats-setup.sh` 自动完成**安装 + 证书 + 配置 + 开机自启 + 防火墙**(nsc 账号步骤会在结尾打印让你执行一次):
```bash
# 域名 + Let's Encrypt + systemd 开机自启
DOMAIN=nats.your-domain.com TLS=letsencrypt PM=systemd ./scripts/nats-setup.sh
# 无域名 + 自签 + pm2
TLS=selfsigned PM=pm2 ./scripts/nats-setup.sh
# Docker(restart=unless-stopped 即开机自启)
PM=docker DOMAIN=nats.your-domain.com TLS=letsencrypt ./scripts/nats-setup.sh
```
变量:`DOMAIN`(可选)、`TLS=letsencrypt|selfsigned`、`PM=systemd|pm2|docker`、`PORT`(4222)、`WS_PORT`(8443)。
跑完后按它打印的 `nsc` 命令做**一次**账号设置即可。下面是手动分步版。
